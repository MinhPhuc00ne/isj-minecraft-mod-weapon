...
public class LargeSwordItemRenderer
extends BlockEntityWithoutLevelRenderer {
    private final EntityModelSet entityModelSet;
    private final ItemStack transformSource;

    public LargeSwordItemRenderer(BlockEntityRenderDispatcher blockEntityRenderDispatcher, EntityModelSet entityModelSet) {
        super(blockEntityRenderDispatcher, entityModelSet);
        this.entityModelSet = entityModelSet;
        this.transformSource = new ItemStack((ItemLike)BossesRiseItems.LARGE_SWORD.get());
    }
    ...
    public void renderByItem(ItemStack itemstack, ItemDisplayContext displayContext, PoseStack poseStack, MultiBufferSource bufferSource, int packedLight, int packedOverlay) {
        Modellarge_sword model = new Modellarge_sword(this.entityModelSet.bakeLayer(Modellarge_sword.LAYER_LOCATION));
        if (model == null) {
            return;
        }
        if (displayContext != ItemDisplayContext.GUI) {
            ...
            poseStack.pushPose();
            Minecraft.getInstance().getItemRenderer().getModel(this.transformSource, null, null, 0).getTransforms().getTransform(displayContext).apply(LargeSwordItemRenderer.isLeftHand(displayContext), poseStack);
            ...
            AnimationState idleState = LargeSwordItem.IDLE_STATES.computeIfAbsent(key, s -> new AnimationState());
            AnimationState swingState = LargeSwordItem.SWING_STATES.computeIfAbsent(key, s -> new AnimationState());
            ...
            VertexConsumer vertexConsumer = ItemRenderer.getFoilBufferDirect((MultiBufferSource)bufferSource, (RenderType)model.renderType(BossesRise.prefix("textures/entities/large_sword.png")), (boolean)false, (boolean)itemstack.hasFoil());
