...
extends SubInstantParticleEmitter {
    ...
    private static final ColorCurve COLOR_CURVE = new ColorCurve(new ColorCurve.Point[]{new ColorCurve.Point(ColorCurve.FloatRGBA.fromARGB(1854074367), 0.0f), new ColorCurve.Point(ColorCurve.FloatRGBA.fromARGB(9958655), 1.0f)}, (subParticle, partial) -> Float.valueOf(((float)subParticle.age + partial.floatValue()) / (float)subParticle.lifeTime + subParticle.random2));

    public static IceWhirlpool2Provider provider(SpriteSet spriteSet) {
        return new IceWhirlpool2Provider(spriteSet);
    }

    protected YetiSmokeImpact2(ClientLevel world, double x, double y, double z, double vx, double vy, double vz, SpriteSet spriteSet) {
        super(world, x, y, z, spriteSet, 50);
    ...
    protected SubParticle makeParticle() {
        ...
        subParticle.setVelocity(new Vec3(1.0, 0.0, 0.0).yRot(angle), subParticle.random2 * 30.0f);
        return subParticle;
    }
    ...
    implements ParticleProvider<SimpleParticleType> {
        ...
        public Particle createParticle(SimpleParticleType typeIn, ClientLevel worldIn, double x, double y, double z, double xSpeed, double ySpeed, double zSpeed) {
            return new YetiSmokeImpact2(worldIn, x, y, z, xSpeed, ySpeed, zSpeed, this.spriteSet);
        }
    }
}
