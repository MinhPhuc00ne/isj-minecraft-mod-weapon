...
public class FrozenFistItemRenderer
extends BlockEntityWithoutLevelRenderer {
    private final EntityModelSet entityModelSet;
    private final ItemStack transformSource;

    public FrozenFistItemRenderer(BlockEntityRenderDispatcher blockEntityRenderDispatcher, EntityModelSet entityModelSet) {
        super(blockEntityRenderDispatcher, entityModelSet);
        this.entityModelSet = entityModelSet;
        this.transformSource = new ItemStack((ItemLike)BossesRiseItems.ICE_GAUNTLET.get());
    }
    ...
    public void renderByItem(ItemStack itemstack, ItemDisplayContext displayContext, PoseStack poseStack, MultiBufferSource bufferSource, int packedLight, int packedOverlay) {
        Modelice_gauntlet model = new Modelice_gauntlet(this.entityModelSet.bakeLayer(Modelice_gauntlet.LAYER_LOCATION));
        if (model == null) {
            return;
        }
        if (displayContext != ItemDisplayContext.GUI) {
            ...
            poseStack.pushPose();
            Minecraft.getInstance().getItemRenderer().getModel(this.transformSource, null, null, 0).getTransforms().getTransform(displayContext).apply(FrozenFistItemRenderer.isLeftHand(displayContext), poseStack);
            ...
            if (displayContext.firstPerson()) {
                ...
                if (FrozenFistItemRenderer.isLeftHand(displayContext)) {
            ...
            AnimationState idleState = IceGauntletItem.IDLE_STATES.computeIfAbsent(key, s -> new AnimationState());
            AnimationState swingState = IceGauntletItem.SWING_STATES.computeIfAbsent(key, s -> new AnimationState());
            ...
            VertexConsumer vertexConsumer = ItemRenderer.getFoilBufferDirect((MultiBufferSource)bufferSource, (RenderType)model.renderType(BossesRise.prefix("textures/entities/ice_gauntlet.png")), (boolean)false, (boolean)itemstack.hasFoil());
