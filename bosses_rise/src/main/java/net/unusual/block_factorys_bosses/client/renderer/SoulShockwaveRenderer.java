...
public class SoulShockwaveRenderer
extends EntityRenderer<SoulShockwaveEntity> {
    ...
    public SoulShockwaveRenderer(EntityRendererProvider.Context context) {
        super(context);
    }

    public void render(SoulShockwaveEntity wave, float entityYaw, float partialTicks, PoseStack poseStack, MultiBufferSource buffer, int packedLight) {
        VertexConsumer vertexconsumer = buffer.getBuffer(RENDER_TYPE);
        poseStack.pushPose();
        poseStack.mulPose(Axis.YP.rotationDegrees(-entityYaw));
        poseStack.scale(6.0f, 6.0f, 6.0f);
        PoseStack.Pose pose = poseStack.last();
        SoulShockwaveRenderer.vertex(vertexconsumer, pose, 1.0f, 1, 0, 1);
        SoulShockwaveRenderer.vertex(vertexconsumer, pose, 0.0f, 1, 1, 1);
        SoulShockwaveRenderer.vertex(vertexconsumer, pose, 0.0f, 0, 1, 0);
        SoulShockwaveRenderer.vertex(vertexconsumer, pose, 1.0f, 0, 0, 0);
        poseStack.popPose();
    ...
    public ResourceLocation getTextureLocation(SoulShockwaveEntity entity) {
