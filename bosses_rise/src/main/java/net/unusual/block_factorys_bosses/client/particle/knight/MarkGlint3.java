...
extends SubSteadyParticleEmitter {
    ...
    private static final ColorCurve COLOR_CURVE = new ColorCurve(new ColorCurve.Point[]{new ColorCurve.Point(ColorCurve.FloatRGBA.fromARGB(16753152), 0.0f), new ColorCurve.Point(ColorCurve.FloatRGBA.fromARGB(-35584), 0.09f), new ColorCurve.Point(ColorCurve.FloatRGBA.fromARGB(-905999104), 0.48f), new ColorCurve.Point(ColorCurve.FloatRGBA.fromARGB(16744704), 1.0f)}, (subParticle, partial) -> Float.valueOf(((float)subParticle.age + partial.floatValue()) / (float)subParticle.lifeTime));

    public static Provider provider(SpriteSet spriteSet) {
        return new Provider(spriteSet);
    }

    protected MarkGlint3(ClientLevel world, double x, double y, double z, double vx, double vy, double vz, SpriteSet spriteSet) {
        super(world, x, y, z, spriteSet, (int)(100.0 * vx), 15, 100);
    ...
    protected float getSubSize(SubParticle subParticle, float partialTick, float ageInSeconds) {
        return SCALE_FACTOR.get(subParticle, partialTick) * 0.2f * this.scale;
    }
    ...
    implements ParticleProvider<SimpleParticleType> {
        ...
        public Particle createParticle(SimpleParticleType typeIn, ClientLevel worldIn, double x, double y, double z, double xSpeed, double ySpeed, double zSpeed) {
            return new MarkGlint3(worldIn, x, y, z, xSpeed, ySpeed, zSpeed, this.spriteSet);
        }
    }
}
