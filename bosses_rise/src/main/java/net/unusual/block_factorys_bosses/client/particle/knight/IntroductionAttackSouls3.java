...
public class IntroductionAttackSouls3
extends SubSteadyParticleEmitter {
    private static final LinearCurve X_FACTOR = new LinearCurve(new float[]{1.0f, 1.0f, 1.0f, 1.0f, 1.0f, 0.96f, 0.84f}, (subParticle, partial) -> Float.valueOf(((float)subParticle.age + partial.floatValue()) / 20.0f), (subParticle, partial) -> Float.valueOf((float)subParticle.lifeTime / 20.0f));
    ...
    private static final ColorCurve COLOR_CURVE = new ColorCurve(new ColorCurve.Point[]{new ColorCurve.Point(ColorCurve.FloatRGBA.fromARGB(-9699352), 0.0f), new ColorCurve.Point(ColorCurve.FloatRGBA.fromARGB(-615190545), 0.91f), new ColorCurve.Point(ColorCurve.FloatRGBA.fromARGB(0xAAF6FF), 1.0f)}, (subParticle, partial) -> Float.valueOf(((float)subParticle.age + partial.floatValue()) / (float)subParticle.lifeTime));

    public static Provider provider(SpriteSet spriteSet) {
        return new Provider(spriteSet);
    }

    protected IntroductionAttackSouls3(ClientLevel world, double x, double y, double z, double vx, double vy, double vz, SpriteSet spriteSet) {
        super(world, x, y, z, spriteSet, 10, 30, 10);
    ...
    protected Vector3f getSubSizeVector(SubParticle subParticle, float partialTick, float ageInSeconds) {
        return new Vector3f(0.8f * X_FACTOR.get(subParticle, partialTick), 0.8f * X_FACTOR.get(subParticle, partialTick) * S_FACTOR.get(subParticle, partialTick), 1.0f);
    }
    ...
    protected SubParticle makeParticle() {
        ...
        SubParticle subParticle = new SubParticle(this, this, this.set, this.getSubParticleLifeTime(), this.random){
            ...
            public Vec3 getLinearAcceleration() {
                return new Vec3(-Math.cos((float)this.age / 20.0f * 720.0f) * -35.0, (double)(-15.0f * this.random2 - 15.0f), -Math.sin((float)this.age / 20.0f * 720.0f) * 30.0);
            }
        ...
        subParticle.pos = pos = new Vec3(5.0, 0.0, 0.0).yRot(angle).add(0.0, 1.0, 0.0);
        subParticle.pos0 = pos;
        subParticle.linearDragCoefficient = 2.0;
        subParticle.setVelocity(new Vec3(0.0, 1.0, 0.0), 35.0f);
        return subParticle;
    }
    ...
    implements ParticleProvider<SimpleParticleType> {
        ...
        public Particle createParticle(SimpleParticleType typeIn, ClientLevel worldIn, double x, double y, double z, double xSpeed, double ySpeed, double zSpeed) {
            return new IntroductionAttackSouls3(worldIn, x, y, z, xSpeed, ySpeed, zSpeed, this.spriteSet);
        }
    }
}
