...
public class WarriorSwordItemRenderer
extends BlockEntityWithoutLevelRenderer {
    private final EntityModelSet entityModelSet;
    private final ItemStack transformSource;

    public WarriorSwordItemRenderer(BlockEntityRenderDispatcher blockEntityRenderDispatcher, EntityModelSet entityModelSet) {
        super(blockEntityRenderDispatcher, entityModelSet);
        this.entityModelSet = entityModelSet;
        this.transformSource = new ItemStack((ItemLike)BossesRiseItems.WARRIOR_SWORD.get());
    }
    ...
    public void renderByItem(ItemStack itemstack, ItemDisplayContext displayContext, PoseStack poseStack, MultiBufferSource bufferSource, int packedLight, int packedOverlay) {
        Modelwarrior_sword model = new Modelwarrior_sword(this.entityModelSet.bakeLayer(Modelwarrior_sword.LAYER_LOCATION));
        if (model == null) {
            return;
        }
        if (displayContext != ItemDisplayContext.GUI) {
            ...
            poseStack.pushPose();
            Minecraft.getInstance().getItemRenderer().getModel(this.transformSource, null, null, 0).getTransforms().getTransform(displayContext).apply(WarriorSwordItemRenderer.isLeftHand(displayContext), poseStack);
            ...
            AnimationState idleState = WarriorSwordItem.IDLE_STATES.computeIfAbsent(key, s -> new AnimationState());
            AnimationState swingState = WarriorSwordItem.SWING_STATES.computeIfAbsent(key, s -> new AnimationState());
            ...
            VertexConsumer vertexConsumer = ItemRenderer.getFoilBufferDirect((MultiBufferSource)bufferSource, (RenderType)model.renderType(BossesRise.prefix("textures/entities/warrior_sword.png")), (boolean)false, (boolean)itemstack.hasFoil());
