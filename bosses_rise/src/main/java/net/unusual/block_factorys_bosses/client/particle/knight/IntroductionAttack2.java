...
public class IntroductionAttack2
extends SubSteadyParticleEmitter {
    private static final LinearCurve X_FACTOR = new LinearCurve(new float[]{0.0f, 1.0f, 1.1f, 1.07f, 1.0f}, (subParticle, partial) -> Float.valueOf(((float)subParticle.age + partial.floatValue()) / 20.0f), (subParticle, partial) -> Float.valueOf((float)subParticle.lifeTime / 20.0f));
    ...
    protected IntroductionAttack2(ClientLevel world, double x, double y, double z, double vx, double vy, double vz, SpriteSet spriteSet) {
        super(world, x, y, z, spriteSet, 16, 7, 100);
    ...
    protected int getSubParticleLifeTime() {
        return Mth.randomBetweenInclusive((RandomSource)this.random, (int)12, (int)14);
    }
    ...
    protected SubParticle makeParticle() {
        ...
        SubParticle subParticle = new SubParticle(this, this, this.set, this.getSubParticleLifeTime(), this.random){
            ...
            public Vec3 getLinearAcceleration() {
                return new Vec3(Math.cos((double)this.age * 360.0) * 5.0, 0.0, Math.sin((double)this.age * 360.0) * 5.0);
            }
            ...
            protected List<Pair<Float, BiConsumer<SubParticle, Float>>> createLifetimeEvents() {
                ...
                for (float f = 0.0f; f <= 0.6f; f += 0.01f) {
            ...
            private void event(SubParticle sub, Float partialTick) {
                Vec3 pos = this.parent.getPos().add(sub.pos);
        ...
        subParticle.pos = pos = new Vec3(1.0, 0.0, 0.0).yRot(angle);
        subParticle.pos0 = pos;
        subParticle.linearDragCoefficient = 4.0;
        subParticle.setVelocity(new Vec3((double)Mth.randomBetween((RandomSource)this.random, (float)-0.2f, (float)0.2f), 1.0, (double)Mth.randomBetween((RandomSource)this.random, (float)-0.2f, (float)0.2f)), 15.0f);
        return subParticle;
    }
    ...
    implements ParticleProvider<SimpleParticleType> {
        ...
        public Particle createParticle(SimpleParticleType typeIn, ClientLevel worldIn, double x, double y, double z, double xSpeed, double ySpeed, double zSpeed) {
            return new IntroductionAttack2(worldIn, x, y, z, xSpeed, ySpeed, zSpeed, this.spriteSet);
        }
    }
}
