...
public class DaggerItemRenderer
extends BlockEntityWithoutLevelRenderer {
    private final EntityModelSet entityModelSet;
    private final ItemStack transformSource;

    public DaggerItemRenderer(BlockEntityRenderDispatcher blockEntityRenderDispatcher, EntityModelSet entityModelSet) {
        super(blockEntityRenderDispatcher, entityModelSet);
        this.entityModelSet = entityModelSet;
        this.transformSource = new ItemStack((ItemLike)BossesRiseItems.DAGGER.get());
    }
    ...
    public void renderByItem(ItemStack itemstack, ItemDisplayContext displayContext, PoseStack poseStack, MultiBufferSource bufferSource, int packedLight, int packedOverlay) {
        Modeldagger model = new Modeldagger(this.entityModelSet.bakeLayer(Modeldagger.LAYER_LOCATION));
        if (model == null) {
            return;
        }
        if (displayContext != ItemDisplayContext.GUI) {
            ...
            poseStack.pushPose();
            Minecraft.getInstance().getItemRenderer().getModel(this.transformSource, null, null, 0).getTransforms().getTransform(displayContext).apply(DaggerItemRenderer.isLeftHand(displayContext), poseStack);
            ...
            AnimationState idleState = DaggerItem.IDLE_STATES.computeIfAbsent(key, s -> new AnimationState());
            AnimationState swingState = DaggerItem.SWING_STATES.computeIfAbsent(key, s -> new AnimationState());
            ...
            VertexConsumer vertexConsumer = ItemRenderer.getFoilBufferDirect((MultiBufferSource)bufferSource, (RenderType)model.renderType(BossesRise.prefix("textures/entities/dagger.png")), (boolean)false, (boolean)itemstack.hasFoil());
