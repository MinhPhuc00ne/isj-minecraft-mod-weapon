...
public class PhaseTransition9
extends SubSteadyParticleEmitter {
    private static final LinearCurve X_FACTOR = new LinearCurve(new float[]{0.54f, 1.0f, 0.54f}, (subParticle, partial) -> Float.valueOf(subParticle.random2), (subParticle, partial) -> Float.valueOf(1.0f));
    private static final LinearCurve SCALE_FACTOR = new LinearCurve(new float[]{1.0f, 1.0f, 0.96f, 0.86f, 0.61f, 0.32f, 0.0f}, (subParticle, partial) -> Float.valueOf(((float)subParticle.age + partial.floatValue()) / (float)subParticle.lifeTime), (subParticle, partial) -> Float.valueOf(1.0f));
    private static final ColorCurve COLOR_CURVE = new ColorCurve(new ColorCurve.Point[]{new ColorCurve.Point(ColorCurve.FloatRGBA.fromARGB(2013331429), 0.0f), new ColorCurve.Point(ColorCurve.FloatRGBA.fromARGB(50687), 1.0f)}, (subParticle, partial) -> Float.valueOf(((float)subParticle.age + partial.floatValue()) / (float)subParticle.lifeTime));

    public static Provider provider(SpriteSet spriteSet) {
        return new Provider(spriteSet);
    }

    protected PhaseTransition9(ClientLevel world, double x, double y, double z, double vx, double vy, double vz, SpriteSet spriteSet) {
        super(world, x, y, z, spriteSet, 100, 15, 100);
    ...
    protected float getSubSize(SubParticle subParticle, float partialTick, float ageInSeconds) {
        return X_FACTOR.get(subParticle, partialTick) * SCALE_FACTOR.get(subParticle, partialTick);
    }
    ...
    protected SubParticle makeParticle() {
        ...
        subParticle.setVelocity(pos, 0.5f);
        return subParticle;
    }
    ...
    implements ParticleProvider<SimpleParticleType> {
        ...
        public Particle createParticle(SimpleParticleType typeIn, ClientLevel worldIn, double x, double y, double z, double xSpeed, double ySpeed, double zSpeed) {
            return new PhaseTransition9(worldIn, x, y, z, xSpeed, ySpeed, zSpeed, this.spriteSet);
        }
    }
}
