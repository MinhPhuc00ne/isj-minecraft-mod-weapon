...
extends SubSteadyParticleEmitter {
    ...
    private static final ColorCurve COLOR_CURVE = new ColorCurve(new ColorCurve.Point[]{new ColorCurve.Point(new ColorCurve.FloatRGBA(0.564f, 0.949f, 1.0f, 0.5f), 0.0f), new ColorCurve.Point(new ColorCurve.FloatRGBA(1.0f, 1.0f, 1.0f, 1.0f), 1.0f)}, (subParticle, partial) -> Float.valueOf(((float)subParticle.age + partial.floatValue()) / (float)subParticle.lifeTime));

    public static IceWhirlpool1Provider provider(SpriteSet spriteSet) {
        return new IceWhirlpool1Provider(spriteSet);
    }

    protected IceWhirlpool1(ClientLevel world, double x, double y, double z, double vx, double vy, double vz, SpriteSet spriteSet) {
    ...
    protected double getSubX(SubParticle subParticle, float partialTick, float ageInSeconds) {
        return Math.cos((double)(180.0f * ageInSeconds) / 1.5 * (double)this.randomValue / 20.0 + (double)(subParticle.random4 * 720.0f)) * (double)C_FACTOR.get(subParticle, partialTick) * (double)V_FACTOR.get(subParticle, partialTick) * 8.0;
    }
    ...
    protected double getSubZ(SubParticle subParticle, float partialTick, float ageInSeconds) {
        return Math.sin((double)(180.0f * ageInSeconds) / 1.5 * (double)this.randomValue / 20.0 + (double)(subParticle.random4 * 720.0f)) * (double)C_FACTOR.get(subParticle, partialTick) * (double)V_FACTOR.get(subParticle, partialTick) * 8.0;
    }
    ...
    implements ParticleProvider<SimpleParticleType> {
        ...
        public Particle createParticle(SimpleParticleType typeIn, ClientLevel worldIn, double x, double y, double z, double xSpeed, double ySpeed, double zSpeed) {
            return new IceWhirlpool1(worldIn, x, y, z, xSpeed, ySpeed, zSpeed, this.spriteSet);
        }
    }
}
