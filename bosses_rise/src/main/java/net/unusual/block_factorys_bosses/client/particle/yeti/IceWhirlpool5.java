...
extends SubSteadyParticleEmitter {
    ...
    private static final LinearCurve X_FACTOR = new LinearCurve(new float[]{1.0f, 1.0f, 0.57f, 0.0f}, (subParticle, partial) -> Float.valueOf(((float)subParticle.age + partial.floatValue()) / (float)subParticle.lifeTime), (subParticle, partial) -> Float.valueOf((float)subParticle.lifeTime / 20.0f));
    private static final LinearCurve V_FACTOR = new LinearCurve(new float[]{2.52f, 2.86f, 2.56f, 2.57f, 2.65f, 2.53f, 1.86f}, (subParticle, partial) -> Float.valueOf(((float)subParticle.age + partial.floatValue()) / (float)subParticle.lifeTime), (subParticle, partial) -> Float.valueOf((float)subParticle.lifeTime / 20.0f));
    ...
    private static final ColorCurve COLOR_CURVE = new ColorCurve(new ColorCurve.Point[]{new ColorCurve.Point(new ColorCurve.FloatRGBA(0.56f, 0.95f, 1.0f, 0.47f), 0.0f), new ColorCurve.Point(new ColorCurve.FloatRGBA(1.0f, 1.0f, 1.0f, 1.0f), 1.0f)}, (subParticle, partial) -> Float.valueOf(((float)subParticle.age + partial.floatValue()) / (float)subParticle.lifeTime));

    public static IceWhirlpool5Provider provider(SpriteSet spriteSet) {
        return new IceWhirlpool5Provider(spriteSet);
    }

    protected IceWhirlpool5(ClientLevel world, double x, double y, double z, double vx, double vy, double vz, SpriteSet spriteSet) {
        super(world, x, y, z, spriteSet, 200, 120, 300);
    ...
    protected double getSubX(SubParticle subParticle, float partialTick, float ageInSeconds) {
        return Math.cos(180.0f * ageInSeconds / 20.0f / 2.0f * this.randomValue + subParticle.random4 * 720.0f) * (double)C_FACTOR.get(subParticle, partialTick) * (double)V_FACTOR.get(subParticle, partialTick) * 10.0;
    }

    @Override
    protected double getSubY(SubParticle subParticle, float partialTick, float ageInSeconds) {
        return subParticle.random1 * 2.0f * ageInSeconds * 3.0f * C_FACTOR.get(subParticle, partialTick);
    }

    @Override
    protected double getSubZ(SubParticle subParticle, float partialTick, float ageInSeconds) {
        return Math.sin(180.0f * ageInSeconds / 20.0f / 2.0f * this.randomValue + subParticle.random4 * 720.0f) * (double)C_FACTOR.get(subParticle, partialTick) * (double)V_FACTOR.get(subParticle, partialTick) * 10.0;
    }
    ...
    public static class IceWhirlpool5Provider
    implements ParticleProvider<SimpleParticleType> {
        private final SpriteSet spriteSet;

        public IceWhirlpool5Provider(SpriteSet spriteSet) {
            this.spriteSet = spriteSet;
        }

        public Particle createParticle(SimpleParticleType typeIn, ClientLevel worldIn, double x, double y, double z, double xSpeed, double ySpeed, double zSpeed) {
            return new IceWhirlpool5(worldIn, x, y, z, xSpeed, ySpeed, zSpeed, this.spriteSet);
        }
    }
}
