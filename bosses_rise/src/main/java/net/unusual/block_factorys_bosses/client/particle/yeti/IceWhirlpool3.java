...
extends SubSteadyParticleEmitter {
    ...
    public static IceWhirlpool3Provider provider(SpriteSet spriteSet) {
        return new IceWhirlpool3Provider(spriteSet);
    }

    protected IceWhirlpool3(ClientLevel world, double x, double y, double z, double vx, double vy, double vz, SpriteSet spriteSet) {
        super(world, x, y, z, spriteSet, 200, 30, 300);
    ...
    protected double getSubX(SubParticle subParticle, float partialTick, float ageInSeconds) {
        return Math.cos((double)(180.0f * ageInSeconds / 20.0f) / 0.5 * (double)this.randomValue + (double)(subParticle.random4 * 720.0f)) * (double)C_FACTOR.get(subParticle, partialTick) * (double)V_FACTOR.get(subParticle, partialTick);
    }

    @Override
    protected double getSubY(SubParticle subParticle, float partialTick, float ageInSeconds) {
        return (double)(subParticle.random1 * 2.0f * ageInSeconds * 3.0f * C_FACTOR.get(subParticle, partialTick)) * 0.5 * 20.0;
    }

    @Override
    protected double getSubZ(SubParticle subParticle, float partialTick, float ageInSeconds) {
        return Math.sin((double)(180.0f * ageInSeconds / 20.0f) / 0.5 * (double)this.randomValue + (double)(subParticle.random4 * 720.0f)) * (double)C_FACTOR.get(subParticle, partialTick) * (double)V_FACTOR.get(subParticle, partialTick);
    }

    @Override
    protected float getSubSize(SubParticle subParticle, float partialTick, float ageInSeconds) {
        return X_FACTOR.get(subParticle, partialTick);
    }
    ...
    public static class IceWhirlpool3Provider
    implements ParticleProvider<SimpleParticleType> {
        private final SpriteSet spriteSet;

        public IceWhirlpool3Provider(SpriteSet spriteSet) {
            this.spriteSet = spriteSet;
        }

        public Particle createParticle(SimpleParticleType typeIn, ClientLevel worldIn, double x, double y, double z, double xSpeed, double ySpeed, double zSpeed) {
            return new IceWhirlpool3(worldIn, x, y, z, xSpeed, ySpeed, zSpeed, this.spriteSet);
        }
    }
}
