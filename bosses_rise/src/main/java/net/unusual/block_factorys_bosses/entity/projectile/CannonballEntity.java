...
implements GeoEntity {
    ...
    public void tick() {
        ...
        double dZ = delta.z;
        for (int i = 0; i < 4; ++i) {
            this.level().addParticle((ParticleOptions)ParticleTypes.SMOKE, this.getX() + dX * (double)i / 4.0, this.getY() + dY * (double)i / 4.0, this.getZ() + dZ * (double)i / 4.0, -dX * 0.02, -(dY + 0.2) * 0.02, -dZ * 0.02);
        }
        ...
        this.setXRot(CannonballEntity.lerpRotation((float)this.xRotO, (float)this.getXRot()));
        this.setYRot(CannonballEntity.lerpRotation((float)this.yRotO, (float)this.getYRot()));
        float f = 0.99f;
        if (this.isInWater()) {
            for (int j = 0; j < 4; ++j) {
    ...
    protected EntityHitResult findHitEntity(Vec3 startVec, Vec3 endVec) {
        KrakenEntity kraken = (KrakenEntity)this.level().getNearestEntity(KrakenEntity.class, TargetingConditions.forNonCombat().range(16.0).ignoreLineOfSight(), null, this.getX(), this.getY(), this.getZ(), this.getBoundingBox().inflate(16.0));
        if (kraken != null && this.level().getEntities((Entity)kraken, kraken.getBoundingBox()).contains((Object)this)) {
            return new EntityHitResult((Entity)kraken, this.position());
        }
    ...
    protected void onHitEntity(EntityHitResult result) {
        ...
        if (this.getOwner() != entity) {
            Level level;
            DamageSource damagesource = this.level().damageSources().source(BossesRiseDamageTypes.CANNONBALL_HIT, (Entity)this, this.getOwner());
            if (entity.hurt(damagesource, 10.0f) && (level = this.level()) instanceof ServerLevel) {
        ...
        if (level instanceof ServerLevel) {
            ...
            level2.explode((Entity)this, this.getX(), this.getY(), this.getZ(), 2.0f, false, Level.ExplosionInteraction.MOB);
            this.playSound((SoundEvent)BossesRiseSounds.CANNON_IMPACT.value(), 1.0f, 1.0f);
            this.discard();
        }
    }
    ...
    public boolean shouldRenderAtSqrDistance(double distance) {
        ...
        return distance < (d0 *= 64.0 * CannonballEntity.getViewScale()) * d0;
    }
    ...
    public static boolean hasCausedDamage(DamageSource damageSource) {
        return damageSource.is(BossesRiseDamageTypes.CANNONBALL_HIT) || damageSource.is(DamageTypes.EXPLOSION) && damageSource.getDirectEntity() instanceof CannonballEntity;
    }
}
